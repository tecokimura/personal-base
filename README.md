...
| 実装計画 | In Progress | MVP・第2フェーズ・フェーズ3初期課題はすべて完了済み。現在はフェーズ3追加課題（PMO-97〜104）を `feat/phase3` ブランチで実装中 |
...
Backlog プロジェクト `PMO_PJPERSONALBASE` でカテゴリ `実装待ち` の課題を順番に実装する。

- 実装ブランチ: `feat/phase3`（課題ごとに `feat/pmo-{番号}-{スラグ}` を切る）
- 課題ごとの最新状態は Backlog が正本。docs にステータスは持たない

### フェーズ 3 完了済み

MVP・第2フェーズ全課題に加え、PMO-84・87・88・89・90・91・92・93・94・95・96・100・105・106・107・108・109 が完了済み。
