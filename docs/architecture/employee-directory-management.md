...
- 論理削除前に、有効な `Employment` を在籍終了状態へ整える
- 論理削除社員は通常一覧から除外する
...
- `email` を MVP 必須にするか
- 論理削除前に有効 `Employment` を自動終了するか、事前に明示操作を必須にするか
